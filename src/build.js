...
const renderLayout = function(styleContent, images, iconsContent, vendorScriptContent, scriptContent) {
  return new Promise(function(resolve, reject) {
    for (const key in require.cache) { delete require.cache[key]; }
    const index = require('./views/index');
    const imagesContent = `window.images = ${JSON.stringify(images)};`
    const page = new Vue({
      components: {
        index,
      },
...
const renderPage = function() {
  ...
  .then(rawHtml => htmlMinifier(rawHtml, {
...
const processPage = () => fs.emptyDir('lib')
  .then(() => fs.copy('src/public', 'lib'))
  .then(() => renderPage())
